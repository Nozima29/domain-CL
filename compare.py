...
def handle_inputs():
    ...
    parser.add_argument('--no-context-spec', action='store_true',
                        help="no XdG or Separate Networks")
    ...
    # -check whether incompatible options are selected
...
if __name__ == '__main__':
    ...
    ER = {}
    ER = collect_all(ER, seed_list, args,
                     name="Experience Replay (budget = {})".format(args.budget))
    args.replay = "none"
    ...
    if not checkattr(args, 'no_agem'):
        ...
        AGEM = {}
        AGEM = collect_all(AGEM, seed_list, args,
                           name="A-GEM (budget = {})".format(args.budget))
        args.replay = "none"
    ...
    if args.scenario == "class" and not args.neg_samples == "current":
        ...
        ICARL = {}
        ICARL = collect_all(ICARL, seed_list, args,
                            name="iCaRL (budget = {})".format(args.budget))
        args.bce = False
        ...
        args.lr = 0.001
        GENCLASS = {}
        GENCLASS = collect_all(GENCLASS, seed_list, args,
    ...
    plot_name = "summary-{}{}-{}".format(args.experiment,
                                         args.contexts, args.scenario)
    ...
    if len(seed_list) > 1:
        sems = [np.sqrt(np.var([ave_acc[seed][id]
                        for seed in seed_list])/(len(seed_list)-1)) for id in ids]
        cis = [1.96*np.sqrt(np.var([ave_acc[seed][id]
                            for seed in seed_list])/(len(seed_list)-1)) for id in ids]
    ...
    for i, name in enumerate(names):
        if len(seed_list) > 1:
            print("{:27s} {:.2f}  (+/- {:.2f}),  n={}".format(name,
                  100*means[i], 100*sems[i], len(seed_list)))
        else:
