...
def test_acc(model, dataset, batch_size=128, test_size=1024, verbose=True, context_id=None, allowed_classes=None,
    ...
    label_correction = 0 if checkattr(model, 'stream_classifier') or (
        allowed_classes is None) else allowed_classes[0]
        ...
        with torch.no_grad():
            if checkattr(model, 'stream_classifier'):
                ...
                scores = model.classify(
                    x.to(device), allowed_classes=allowed_classes)
...
def test_all_so_far(model, datasets, current_context, iteration, test_size=None, no_context_mask=False,
    ...
    if current_context is None:
        current_context = i+1
    average_precs = sum([precs[context_id]
                        for context_id in range(current_context)]) / current_context
    ...
    if plotting_dict is not None:
        for i in range(n_contexts):
            plotting_dict['acc per context']['context {}'.format(
                i+1)].append(precs[i])
...
def show_samples(model, config, pdf=None, visdom=None, size=32, pdf_title="Generated images", visdom_title="Samples"):
    ...
    image_tensor = sample.view(-1, config['channels'],
                               config['size'], config['size']).cpu()
    if config['normalize']:
...
def show_reconstruction(model, dataset, config, pdf=None, visdom=None, size=32, context=None):
    ...
    image_tensor = comparison.view(-1,
                                   config['channels'], config['size'], config['size'])
