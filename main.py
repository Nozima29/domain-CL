...
def run(args, verbose=False):
    ...
    for network_id in range(n_networks):
        model_to_set = getattr(model, 'context{}'.format(network_id+1)) if checkattr(args, 'separate_networks') else (
            getattr(model, 'vae{}'.format(network_id)) if checkattr(
                args, 'gen_classifier') else model
        )
    ...
    model.neg_samples = args.neg_samples if hasattr(
        args, 'neg_samples') else "all"

    # Print some model-characteristics on the screen
    if verbose:
        if checkattr(args, 'gen_classifier') or checkattr(args, 'separate_networks'):
            message = '{} copies of:'.format(len(train_datasets))
            utils.print_model_info(model.vae0 if checkattr(
                args, 'gen_classifier') else model.context1, message=message)
        else:
    ...
    if isinstance(model, ContinualLearner) and checkattr(args, 'xdg') and args.gating_prop > 0.:
        ...
        for context_id in range(args.contexts):
            ...
            for i in range(model.fcE.layers):
                ...
                gated_units = np.random.choice(n_units, size=int(
                    args.gating_prop * n_units), replace=False)
    ...
    if isinstance(model, ContinualLearner) and use_fisher:
        ...
        model.fisher_labels = args.fisher_labels if hasattr(
            args, 'fisher_labels') else 'all'
        model.fisher_batch = args.fisher_batch if hasattr(
            args, 'fisher_batch') else 1
        ...
        if checkattr(args, 'fisher_init'):
            ...
            # -> makes that already on the first context regularization will happen
            model.context_count = 1
    ...
    train_gen = True if (args.replay == "generative" and not checkattr(
        args, 'feedback')) else False
    if train_gen:
        if verbose:
        ...
        if generator.optim_type in ("adam", "adam_reset"):
            generator.optimizer = optim.Adam(
                generator.optim_list, betas=(0.9, 0.999))
    ...
    if isinstance(model, MemoryBuffer) and use_memory_buffer:
        ...
        model.sample_selection = args.sample_selection if hasattr(
            args, 'sample_selection') else 'random'
    ...
    no_samples = (checkattr(args, "no_samples")
    ...
    context_cbs = [
        cb._eval_cb(log=args.iters, test_datasets=test_datasets, plotting_dict=plotting_dict,
                    visdom=visdom if checkattr(args, 'prototypes') or checkattr(
                        args, 'gen_classifier') else None,
    ...
    baseline = 'joint' if checkattr(args, 'joint') else (
        'cummulative' if checkattr(args, 'cummulative') else 'none')
    ...
    if args.train:
        ...
        train_fn = train_cl  # if checkattr(args, 'fromp') else (
        #     train_gen_classifier if checkattr(
        #         args, 'gen_classifier') else train_cl
        # )
        ...
        train_fn(
            ...
            generator=generator, gen_iters=args.g_iters if hasattr(
                args, 'g_iters') else args.iters,
        ...
        if args.time:
            ...
            time_file = open(
                "{}/time-{}.txt".format(args.r_dir, param_stamp), 'w')
        ...
        if args.save:
            ...
            utils.save_checkpoint(
                model, args.m_dir, name=save_name, verbose=verbose)
    else:
    ...
    for i in range(args.contexts):
        acc = evaluate.test_acc(
            model, test_datasets[i], verbose=False, test_size=None, context_id=i, allowed_classes=list(
                range(config['classes_per_context']*i,
